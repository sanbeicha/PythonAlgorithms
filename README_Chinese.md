...
**冒泡排序**, sometimes referred to as *sinking sort*, is a simple sorting algorithm that repeatedly steps through the list to be sorted, compares each pair of adjacent items and swaps them if they are in the wrong order. The pass through the list is repeated until no swaps are needed, which indicates that the list is sorted.
...
**插入排序** is a simple sorting algorithm that builds the final sorted array (or list) one item at a time. It is much less efficient on *large* lists than more advanced algorithms such as quicksort, heapsort, or merge sort.
...
**快速排序** (sometimes called *partition-exchange sort*) 是一种非常高效的排序算法, serving as a systematic method for placing the elements of an array in order.
...
**桶排序**, or _bin sort_, is a sorting algorithm that works by distributing the elements of an array into a number of buckets. Each bucket is then sorted individually, either using a different sorting algorithm, or by recursively applying the bucket sorting algorithm.
...
**并归排序** (also commonly spelled *mergesort*) is an efficient, general-purpose, comparison-based sorting algorithm. Most implementations produce a stable sort, which means that the implementation preserves the input order of equal elements in the sorted output. Mergesort is a divide and conquer algorithm that was invented by John von Neumann in 1945.
...
**堆排序** is a _comparison-based_ sorting algorithm. It can be thought of as an improved selection sort. It divides its input into a sorted and an unsorted region, and it iteratively shrinks the unsorted region by extracting the largest element and moving that to the sorted region.
...
**选择排序** 把输入的列表分为两个亚组: 一部分是已经从左到右（或者从右到左）排好序的，另一部分等着被排序。一开始，已经排好序的亚组为空，未排序的亚组是全部的待排序列表。算法的步骤是，首先在未排序的亚组里找到最小值（或者最大值，取决于你的排序要求），把它跟未排序亚组最左端的元素互换，然后把亚组的界限标记右移一位。
...
**希尔排序** 是更为一般化的插入排序，允许远处的元素进行交换。The idea is to arrange the list of elements so that, starting anywhere, considering every nth element gives a sorted list.  Such a list is said to be h-sorted.  Equivalently, it can be thought of as h interleaved lists, each individually sorted.
...
- 快速排序的速度很快，但是难以实现。
- 冒泡排序速度很慢，但是容易实现。对于较小数据集，冒泡排序因为更好实现，会是一个更好的选择。但是对于较大的数据集，快速排序带来的速度提升值得用复杂的算法实现。
...
**Tabu search** uses a local or neighborhood search procedure to iteratively move from one potential solution ${\displaystyle x}$  to an improved solution ${\displaystyle x'}$ in the neighborhood of ${\displaystyle x}$ , until some stopping criterion has been satisfied (generally, an attempt limit or a score threshold). Local search procedures often become stuck in poor-scoring areas or areas where scores plateau. In order to avoid these pitfalls and explore regions of the search space that would be left unexplored by other local search procedures, tabu search carefully explores the neighborhood of each solution as the search progresses. The solutions admitted to the new neighborhood, ${\displaystyle N^{}(x)}$ , are determined through the use of memory structures. Using these memory structures, the search progresses by iteratively moving from the current solution ${\displaystyle x}$ to an improved solution ${\displaystyle x'}$ in ${\displaystyle N^{}(x)}$ .
